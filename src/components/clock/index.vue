<template>
  <div class="card">
    ...
    <div class="needles">
      <span
        class="h"
        :style="{ transform: `translate(-50%, -100%) rotateZ(${hours}deg)` }"
      ></span>
      <span
        class="m"
        :style="{ transform: `translate(-50%, -100%) rotateZ(${minutes}deg)` }"
      ></span>
      <span
        class="s"
        :style="{ transform: `translate(-50%, -80%) rotateZ(${seconds}deg)` }"
      ></span>
...
export default {
  ...
  methods: {
    updateTime() {
      const now = new Date()
      this.hours = (now.getHours() % 12) * 30 + now.getMinutes() * 0.5
...
.card {
  ...
  box-shadow:
    ...
    -1px -1px 5px rgba(0, 0, 0, 0.473),
    inset -1px -1px 5px rgba(0, 0, 0, 0.473);
}
